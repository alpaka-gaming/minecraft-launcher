...
namespace Infrastructure.References
{
    public class WUTokenHelper
    {
        public static string GetWUToken()
        {
            ...
            {
                string token;
                var status = GetWUToken(out token);
    ...
    public class WUProtocol
    {
        ...
        {
            var tickets = new XElement(wuws + "WindowsUpdateTicketsToken",
        ...
        public string[] ExtractDownloadResponseUrls(XDocument doc)
        {
            ...
            var result = doc.XPathSelectElement("/s:Envelope/s:Body/wu:GetExtendedUpdateInfo2Response/wu:GetExtendedUpdateInfo2Result", nsmgr);
            if (result == null)
                return new string[0];
